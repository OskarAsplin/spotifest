...
export const getMaxArtistsInFullLineupWidth = (bigScreen: boolean, smallScreen: boolean, maxBigScreen: number) => {
    const vw = Math.min(document.documentElement.clientWidth || 0, window.innerWidth || 0);
    const outerMargin = smallScreen ? 16 : 32;
    return bigScreen ? Math.min(maxBigScreen, Math.floor((0.99 * (vw - outerMargin)) / 100)) : Math.max(3, Math.floor((0.99 * (vw - outerMargin)) / 75));
}
